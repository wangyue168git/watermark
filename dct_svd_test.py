...
def embed_watermark(img, watermark):
    ...
    for i, block in enumerate(dct_blocks):
        if i < len(watermark_array):
            ...
            s_new = np.zeros_like(block)
            s_new[:s.shape[0], :s.shape[0]] = np.diag(s)
            s_new[0, 0] += watermark_array[i]
            block = U @ s_new @ Vt
            dct_blocks[i] = block
        else:
            break
...
def extract_watermark(img, watermark_length):
    ...
    for block in dct_blocks:
        ...
        s_new = np.zeros_like(block)
        s_new[:s.shape[0], :s.shape[0]] = np.diag(s)
        watermark_array.append(int(round(s_new[0, 0])) % 2)
    watermark_bits = ''.join([str(bit) for bit in watermark_array])
    watermark_str = ''.join(chr(int(watermark_bits[i:i+8], 2)) for i in range(0, len(watermark_bits), 8))
    watermark_unicode = watermark_str.encode('unicode_escape').decode('utf-8')
